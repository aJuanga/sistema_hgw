            ...
            <div class="p-6">
                ...
                <div class="flex gap-3">
                    <button type="button"
                            ...
                            class="flex-1 rounded-xl border border-slate-700 bg-slate-800 px-4 py-3 text-sm font-semibold text-slate-200 transition hover:bg-slate-700 disabled:opacity-50 disabled:cursor-not-allowed">
                        Cancelar
                    </button>
                    <button type="button"
                            @click="submitRating()"
                            :disabled="isSubmitting"
                            class="flex-1 inline-flex items-center justify-center space-x-2 rounded-xl bg-gradient-to-r from-emerald-500 to-emerald-600 px-4 py-3 text-sm font-bold text-white shadow-lg shadow-emerald-500/30 transition hover:from-emerald-600 hover:to-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed">
        ...
        function dashboardApp() {
            return {
                ...
                async submitRating() {
                    ...
                    try {
                        const response = await fetch('{{ route("client.cafe-rating.store") }}', {
                            method: 'POST',
                            headers: {
                ...
                async init() {
                    ...
                    try {
                        const response = await fetch('{{ route("client.cafe-rating.check") }}');
                        const data = await response.json();

                        if (data.success && data.hasRated && data.rating) {
                            this.rating = data.rating.rating;
